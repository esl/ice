...
eval({b_apply, E0, Eis}, I, E, K, D, W) ->
  ...
  case ice_sets:union_d(E0Eis) of
    ...
    false ->
      ...
      K2 = ice_sets:perturb(HParams, K1),
...
eval({v_apply, E0, Eis}, I, E, K, D, W) ->
  ...
  case ice_sets:union_d(E0Eis) of
    ...
    false ->
      [{closure, K1, D1, Body, P}|Eis1] = E0Eis,
      HParams = map_params(P, Eis1),
      K2 = ice_sets:perturb(ice_sets:perturb(K, HParams), K1),
...
eval({wheredim, Q, Rho, E0, XiEis}, I, E, K, D, W) ->
  ...
  eval(E0, I, E, K3, ice_sets:union(D, Phis), W);
  ...
  case ice_sets:is_k(D0) andalso ice_sets:subset(D0, ice_sets:domain(K)) of
    true ->
      case ice_sets:difference(D0, D) of
        [] ->
          %% Given that a base application ignores the context at the time of
          %% application, this clause can occur when referring to a dimension
          %% external to the base application. 
          io:format("The dimensions which are missing whilst evaluating ~p " ++
                    "are already known dimensions in the parent scope.~n", [Xi]),
          {error, loop_detected, D0};
        _ ->
